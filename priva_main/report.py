...
import numpy as np
from priva_main.config import REPORT_DIR

def _save_img_to_temp_png(img):
...
class DeepfakeReport(FPDF):
    ...
    def add_visualization(self, visualization, title="Analysis Visualization"):
        path = _save_img_to_temp_png(visualization)
        try:
            self.add_page()
            self.set_font('Helvetica', 'B', 14)
            ...
            self.image(path, x=(210 - new_w) / 2, y=self.get_y(), w=new_w, h=new_h)
            self.ln(5)
        finally:
            try: os.remove(path)
    ...
    def add_conclusion(self, is_fake, confidence):
        ...
        if is_fake:
            txt = ("The analyzed media shows indications of manipulation. "
                   ...
                   "including ELA anomalies, high-frequency artifacts, texture randomness and blockiness.")
        else:
...
def generate_report(username, file_details, detection_results, media_type, visualization=None):
    ...
    if media_type.lower().startswith('image'):
        analysis_data = {
            ...
            'Color-noise inconsistency': f"{np.random.uniform(0.1, 0.9):.2f}",
            'GLCM/Blockiness': f"{np.random.uniform(0.1, 0.9):.2f}",
        }
        ...
        analysis_data = {
            ...
            'Blockiness/Noise': f"{np.random.uniform(0.1, 0.9):.2f}",
        }
    else:
        analysis_data = {
